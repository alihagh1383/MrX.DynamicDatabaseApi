    ...
    public class RoleEndpoint
    {
        public static Return RoleGet(DBWUser UM, DBWRoles RM, HttpContext http, string RoleName)
        {
            ...
            bool ExistRole(UsersTable user)
            {
                if (user.Roles.Any(p => p.Name == RoleName))
                    return true;
                else foreach (var u in user.Childs)
                    {
                        if (ExistRole(u)) return true;
                    }
                return false;
            }
        ...
        public static Return RoleAddField(DBWUser UM, DBWRoles RM, DBWTabels TM, HttpContext http, string RoleName, string Table, string Field, string Mode)
        {
            if (RoleName.StartsWith('_'))
                return Return.Invalid(Return.Loc.Endpoint, "Role Name Can Not Start With {_}");
            if (RoleName.StartsWith("Table"))
        ...
        Do:
            {
                switch (Mode.ToLower())
                {
                    case "add":
                        {
                            var F = User.Roles.Select(p => p.FieldsAddRole.First(p => p.Name == Field && p.Table.Name == Table)).First();
                            if (F == null)
                    ...
                    case "delete":
                        {
                            var F = User.Roles.Select(p => p.FieldsDeleteRole.First(p => p.Name == Field && p.Table.Name == Table)).First();
                            if (F == null)
                                return Return.AccessDeny(Return.Loc.Endpoint, $"You Can Not Cheange Role {Table}.{Field}");
                    ...
                    case "read":
                        {
                            var F = User.Roles.Select(p => p.FieldsReadRole.First(p => p.Name == Field && p.Table.Name == Table)).First();
                            if (F == null)
                                return Return.AccessDeny(Return.Loc.Endpoint, $"You Can Not Cheange Role {Table}.{Field}");
                    ...
                    case "update":
                        {
                            var F = User.Roles.Select(p => p.FieldsUpdateRole.First(p => p.Name == Field && p.Table.Name == Table)).First();
                            if (F == null)
                                return Return.AccessDeny(Return.Loc.Endpoint, $"You Can Not Cheange Role {Table}.{Field}");
                            if (!R.FieldsAddRole.Contains(F)) R.FieldsAddRole.Add(F);
                            if (!R.TabelsAddRole.Contains(T)) R.TabelsAddRole.Add(T);
                ...
                RM.Update(R);
                return Return.Sucsses(Return.Loc.Endpoint, $"Field {Field} In Table {Table} Add To Role {R.Name} By Mode {Mode}");
            }
        }
        public static Return RoleRemoveField(DBWUser UM, DBWRoles RM, HttpContext http, string RoleName, string Table, string Field, string Mode)
        {
            if (RoleName.StartsWith('_'))
                return Return.Invalid(Return.Loc.Endpoint, "Role Name Can Not Start With {_}");
            if (RoleName.StartsWith("Table"))
        ...
        Do:
            {
                ...
                RM.Update(R);
                return Return.Sucsses(Return.Loc.Endpoint, $"Field {Field} In Table {Table} Remove From Role {R.Name} By Mode {Mode}");
            }
        }
        public static Return RoleAddTable(DBWUser UM, DBWTabels TM, DBWRoles RM, HttpContext http, string RoleName, string Table, string Mode)
        {
            if (RoleName.StartsWith('_'))
                return Return.Invalid(Return.Loc.Endpoint, "Role Name Can Not Start With {_}");
            if (RoleName.StartsWith("Table"))
        ...
        public static Return RoleRemoveTable(DBWUser UM, DBWRoles RM, HttpContext http, string RoleName, string Table, string Mode)
        {
            if (RoleName.StartsWith('_'))
                return Return.Invalid(Return.Loc.Endpoint, "Role Name Can Not Start With {_}");
            if (RoleName.StartsWith("Table"))
