...
namespace MrX.DynamicDatabaseApi.Api.Endpoint
{
    public class UserEndpoint
    {
        ...
        public static Return UserAdd(DBWUser UM, string UserName, string Password, HttpContext http)
        {
            ...
            {
                Parent = User,
                Password = MrX.Security.PasswordHash.Hash(Password),
                UserName = UserName
            });
        ...
        DO:
            return Return.Sucsses(Return.Loc.Endpoint, $"User By Name {UserName}", new { U.Name, U.Roles, U.UserName, U.PhoneNumber, U.EmailAddress, U.Id, U.Data, });
        }
        public static Return UserMy(DBWUser UM, HttpContext http)
        {
            var User = UM.Find(p => p.UserName == http.User.Identity!.Name);
            var U = User!;
            return Return.Sucsses(Return.Loc.Endpoint, $"User By Name {U.UserName}", new { U.Name, U.Roles, U.UserName, U.PhoneNumber, U.EmailAddress, U.Id, U.Data, });
        }
        public static Return UserList(DBWUser UM, HttpContext http)
        {
            ...
            return Return.Sucsses(Return.Loc.Endpoint, "Users", c.Select(p => new { p.UserName, p.EmailAddress, p.Id, p.CreatedDate, p.Name, p.Parent, p.PhoneNumber, p.Roles }));
        ...
        DO:
            UM.Remove(U);
            return Return.Sucsses(Return.Loc.Endpoint, $"User By Name {UserName} Is Remove", new { U.Name, U.Roles, U.UserName, U.PhoneNumber, U.EmailAddress, U.Id, U.Data, });

        }
        public static Return UserUpdate(DBWUser UM, string UserName, string Name, string Phone, string Email, HttpContext http)
        {
            var User = UM.Find(p => p.UserName == http.User.Identity!.Name);
            var U = UM.First(p => p.UserName == UserName);
            if (U == null)
        ...
        DO:
            ...
            U.Name = Name;
            UM.Update(U);
            return Return.Sucsses(Return.Loc.Endpoint, $"User By Name {UserName} Updated", new { U.Name, U.Roles, U.UserName, U.PhoneNumber, U.EmailAddress, U.Id, U.Data, });
        }
        ...
        public static Return UserUpdatePassword(DBWUser UM, string UserName, string NewPassword, string? password, HttpContext http)
        {
            var User = UM.Find(p => p.UserName == http.User.Identity!.Name);
            var U = UM.First(p => p.UserName == UserName);
            if (U == null)
            ...
            return Return.AccessDeny(Return.Loc.Endpoint, "You Are Not User Or Creator Or Parent");
        DO:
            U.Password = MrX.Security.PasswordHash.Hash(NewPassword);
            UM.Update(U);
            return Return.Sucsses(Return.Loc.Endpoint, $"User By Name {UserName}", new { U.Name, U.Roles, U.UserName, U.PhoneNumber, U.EmailAddress, U.Id, U.Data, });
        }
