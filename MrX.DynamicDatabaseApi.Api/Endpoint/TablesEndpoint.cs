...
public class TablesEndpoint
{
            ...
            return Return.Sucsses(Return.Loc.Endpoint, $"Table {Name}", new
            {
                ...
                AddRoles = T.Filds.Where(f =>
                {
                    return
                        ...
                        ((f?.IfOneRole ?? false) && T.Filds.Any(p =>
                            p.AddRoles.Any(p => p.Users.Any(p => p.UserName == User!.UserName))));
                }),
                ...
                DeleteRoles = T.Filds.Where(f =>
                {
                    return
                        ...
                        ((f?.IfOneRole ?? false) && T.Filds.Any(p =>
                            p.DeleteRoles.Any(p => p.Users.Any(p => p.UserName == User!.UserName))));
                }),
                UpdateRoles = T.Filds.Where(f =>
                {
                    return
                        ...
                        ((f?.IfOneRole ?? false) && T.Filds.Any(p =>
                            p.UpdateRoles.Any(p => p.Users.Any(p => p.UserName == User!.UserName))));
                })
            });
    ...
    public static Return TablesAddField(string Name, FieldsTable Field, DBWUser UM, DBWTabels TM, HttpContext http)
    {
        var T = TM.First(t => t.Name == Name);
    ...
    public static Return TablesUpdateField(string Name, FieldsTable Field, DBWUser UM, DBWTabels TM, HttpContext http)
    {
        var T = TM.First(t => t.Name == Name);
        if (T == null)
            return Return.ThisExist(Return.Loc.Endpoint, $"Table {Name} Not Exist");
    ...
    public static Return TablesGetField(string Name, string Field, DBWUser UM, DBWTabels TM, HttpContext http)
    {
        ...
        if (F.ReadRoles.Any(p => p.Users.Any(p => p.UserName == User!.UserName)))
            return Return.Sucsses(Return.Loc.Endpoint, "Field Found",
                new
                {
                    F.Name, F.Auto, F.Value, F.Type, F.Disable, F.IfOneRole, F.IsPublic, F.IsUnique, F.Null, F.Show
                });
